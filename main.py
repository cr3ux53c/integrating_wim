...
from os.path import join, basename, splitext, exists, getsize, dirname
from wim import WIM
...
if __name__ == '__main__':
    ...
    if cfg['extract_iso']:
        iso = pycdlib.PyCdlib()
        iso_paths = list(sorted(iglob(join(cfg['src_dir'], '**/*.iso'), recursive=True)))
                    ...
                    for dir, dirlist, filelist in iso.walk(udf_path='/'):
                        for filepath in filelist:
                            if filepath == cfg['target_wim']:
                                ...
                                iso.get_file_from_iso(udf_path='/'.join([dir, filepath]), local_path=join(cfg['dst_dir'], splitext(basename(iso_path))[0] + splitext(cfg['target_wim'])[1]))
                    iso.close()
    ...
    '''
    if cfg['export_wim']:
        wim_paths = list(sorted(iglob(join(cfg['dst_dir'], '*' + splitext(cfg['target_wim'])[1]))))
        if not len(wim_paths):
            ...
            for idx, wim in enumerate(wim_indexes):
                ...
                #     if wim == wim_indexes[i]:
                #         duplicated = True
                # print(f'{Fore.CYAN if idx % 2 else Fore.GREEN}[{idx+1}]\t{wim}{Fore.RED + " (Duplicated)" if duplicated else ""}{Style.RESET_ALL}')
            ...
            # Export using DISM
            for idx, wim in enumerate(tqdm(wim_indexes, desc=f'{Fore.YELLOW}Exporting WIM{Style.RESET_ALL}', leave=True, colour='green', dynamic_ncols=True)):
                ...
                # tqdm.write(f'\n{Fore.RED + "(Skipped due to duplication) " + Fore.WHITE if duplicated else ""}{wim}\n{Style.RESET_ALL}')
                tqdm.write(f'\n{wim}\n{Style.RESET_ALL}')
                os.makedirs(dirname('./' if dirname(cfg['dst_wim_path']) == '' else cfg['dst_wim_path']), exist_ok=True)
                # if cfg['optimize_duplicate'] and duplicated:
                    # pass
                # else:
